...
namespace argh
{
   ...
   public:
      ...
      std::vector<std::pair<std::string, std::string>>  const& params()   const { return params_;   }
   ...
   inline void parser::parse(int argc, const char* const argv[], int mode /*= PREFER_FLAG_FOR_UNREG_OPTION*/)
   {
      ...
      for (auto i = 0u; i < args_.size(); ++i)
      {
         ...
         if (!(mode & NO_SPLIT_ON_EQUALSIGN))
         {
            ...
            {
               params_.emplace_back(name.substr(0, equalPos), name.substr(equalPos + 1));
               continue;
            }
         }
         ...
         {
            params_.emplace_back(name, args_[i + 1]);
   ...
   inline string_stream parser::operator()(std::string const& name) const
   {
       for (const auto& param : params_)
           if (param.first == trim_leading_dashes(name))
               return string_stream(param.second);
